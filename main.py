...
print(f"Menor elemento por linha: {matrix.min(axis=1)}")
...
# product between matrix and its transposed
print(f"Matriz x Matriz transposta + 5:\n{matrix @ transposed_and_five}\n")

# filter matrix
filter = (matrix > 0) & (matrix < 20)
print(
    f"Os elementos positivos da matriz menores que 20 são: {matrix[filter]}\n")
